...
bool DPLLsolver_rand_var2::backTrackAlg()
{
	while (true)
	{
...
void DPLLsolver_rand_var2::createKNFfromDIMACS(const std::string& DIMACS_filepath)
{
...
bool DPLLsolver_rand_var2::chooseVarAlg()
{
	int choosenVar = -1;

	for (int i = 0; i < varCount && choosenVar == -1; i++)
	{
		if ((weightOfColumnsM0[i] || weightOfColumnsM1[i]) && !result[i])
			choosenVar = i;
	}
	...
	bool varVal = true;
	if (weightOfColumnsM0[choosenVar])
	{
		varVal = false;
		if (weightOfColumnsM1[choosenVar] && std::rand() % 100 > 50)
			varVal = true;
	}
...
bool DPLLsolver_rand_var2::deduceAlg()
{
...
void DPLLsolver_rand_var2::setVarVal(int var, bool varVal)
{
	result[var] = 1;
	StackData& stack_top = S.top();
	if (varVal)
